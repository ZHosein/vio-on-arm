...
inline void log3DPoint(const rerun::RecordingStream& rec, const gtsam::Point3& point, const int frameNum, const std::string& entity, const int color=1) {
    ...
    std::vector<rerun::Position3D> points = {
        rerun::Position3D(static_cast<float>(point.x()), static_cast<float>(point.y()), static_cast<float>(point.z()))
    };
...
inline void logPose(const rerun::RecordingStream& rec, const gtsam::Pose3& pose, const int frameNum, const std::string& entity) {
