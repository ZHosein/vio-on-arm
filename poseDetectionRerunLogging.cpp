...
void detectPoseLog () {
    ...
    rec.log("world/camera/image",
        rerun::Image(
            reinterpret_cast<const uint8_t*>(image.data),  // Raw pixel data first
            {static_cast<uint32_t>(image.cols), static_cast<uint32_t>(image.rows)},  // Dimensions
            rerun::datatypes::ColorModel::RGB  // Explicitly specify color model
        ));
